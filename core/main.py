...
PLUGIN_DIR = os.path.expanduser("~/.termux_doctor/plugins")
...
def print_banner(ai_status):
    ...
    print(f"{G} .   .   {Y}=========================={RS}")
    print(f"{G}/ \ / \  {Y} TERMUX-DOCTOR v3.4.1{RS}")
    print(f"{G}\  X  /  {Y} STATUS: {status}{RS}")
    print(f"{G} \/ \/   {Y}=========================={RS}")
    print(f"{Y} Type '?' for the dynamic manual.{RS}\n")
...
def main():
    ...
    while True:
        ...
        # --- DYNAMIC MANUAL ('?') ---
        if cmd == '?':
            print(f"\n{Y}--- SYSTEM COMMANDS ---{RS}")
            print(f"{G}ai on/off{RS} - Toggle Cloud | {G}clear{RS} - Reset UI")
